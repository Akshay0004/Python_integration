import os
print("\t\t\tWelcome to hadoop cluster set up")
print("\t\t\t------------------------------------")
print(''' \t\t\tFor setting hadoop cluster follow the steps mentioned below:
...
count_datanode = int(input("\t\t\tHow many datanode do you want to configure: "))

for i in range(0,count_datanode):
	d_ip = input("\t\t\tIP at which u going to configure datanode{}:".format(i+1))
...
Client_IP = []
count_client = int(input("\t\t\tHow many clients do you want to configure: "))
for i in range(0,count_client):
	c_ip = input("\t\t\tIP at which u going to configure client:")
